...
- `utils::globalVariables()` no longer declares `%>%` or `:=`. Both are imported,
  so neither entry ever suppressed anything. Only `.` is declared now.


# Version 2026.8.20

- `R/.DS_Store` is removed. A macOS Finder artefact was committed inside `R/`,
  so it shipped in the tarball. `.gitignore` now covers it.
